...
export class Breadcrumbs {
  ...
  instrumentConsole() {
    ...
    let realtimeLogFilterFunctions = this.options.realtimeLog
      ? ['warn', 'error', 'info']
      : [];
    ...
    watchFunctions.forEach(function(level) {
      ...
      fill(global.console, level, function(originalConsoleLevel) {
        return function(...args) {
          if (consoleFilterFunctions.indexOf(level) > -1) {
            ...
            if (level === 'assert') {
              if (args[0] === false) {
                ...
                breadcrumbData.data.extra.arguments = serializeObject(
                  args.slice(1),
                  2,
                );
              }
          ...
          if (
            realtimeLogFilterFunctions.indexOf(level) > -1 &&
            realtimeLogManager
          ) {
            let _level = level;
            if (_level === 'log') {
            ...
            realtimeLogManager[_level] &&
              realtimeLogManager[_level].apply(realtimeLogManager, args);
          }
  ...
  setupOnce() {
    if (
      ...
      (this.options.realtimeLog && this.realtimeLogManager)
    ) {
